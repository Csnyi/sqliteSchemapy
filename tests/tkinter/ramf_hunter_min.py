...
logging.basicConfig(
    filename="ramf_hunter_min.log",  # Naplófájl neve
...
class RamfApp(ctk.CTk):
    def __init__(self):
        ...
        self.box_down = ctk.CTkFrame(self)
        self.box_down.grid(row=2, column=1, padx=(20, 0), pady=(20, 0), sticky="nsew")
        ...
        self.spect_button_geoups = ctk.CTkButton(self.tabview.tab("Spectrum Report"), text="Channel Groupes", command=lambda: self.open_toplevel("Play List", self.toplevel_update_groups))
        self.spect_button_geoups.grid(row=3, column=0, padx=20, pady=20, sticky="nsew")
        self.spect_button_open_report = ctk.CTkButton(self.tabview.tab("Spectrum Report"), text="Open Report", command=lambda: self.open_toplevel("Report", self.toplevel_update_report))
    ...
    def get_reportlist(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                report_name = self.report_list[new_keys[0]]
                self.get_report(report_name)
    ...
    def toplevel_update_report(self, window):
        if window.winfo_exists():
            self.report_frame = ctk.CTkFrame(window)
            self.report_frame.pack(padx=5, pady=5, fill="both", expand=True)
            self.report_labels = self.create_labels(self.report_frame, REPORT_LABELS_FIELDS)
